...
WHERE actor_id in
    ...
    (select actor_id
    from actor
    where first_name = "HARPO" and last_name = "WILLIAMS") as ID
	);
...
SELECT count(*) as Number_of_times_Rented, f.title
from film f
...
select ct.name, sum(p.amount) as TOTAL
from payment p
...
create view top_5_genres_bygross as 
select ct.name, sum(p.amount) as TOTAL
from payment p
inner join rental r on r.rental_id = p.rental_id
inner join inventory i on i.inventory_id = r.inventory_id
...
select  * from top_5_genres_bygross;

-- 8c. You find that you no longer need the view top_five_genres. Write a query to delete it.
drop view top_5_genres_bygross;